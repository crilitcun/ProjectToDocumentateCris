...
public class PlanetTest {
    ...
    void checkFieldsSanity() {
        assertEquals(11, Planet.class.getDeclaredFields().length);
    ...
    void checkMethodsSanity() {
        assertEquals(18, Arrays.stream(Planet.class.getDeclaredMethods()).filter(m -> Modifier.isPublic(m.getModifiers())).toList().size());
        assertEquals(0, Arrays.stream(Planet.class.getDeclaredMethods()).filter(m -> Modifier.isPrivate(m.getModifiers())).toList().size());
